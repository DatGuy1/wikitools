...
class TestWiki(unittest.TestCase):
    ...
    def test_Namespace_combination(self):
        self.assertEqual(
            self.site.NS_MAIN | self.site.NS_TEMPLATE | self.site.NS_USER, "0|10|2"
        )
...
class TestPage(unittest.TestCase):
    ...
    def test_constructor_pageid_check_true(self):
        ...
        with self.assertRaises(wikitools.exceptions.WikiError):
            page.Page(self.site, pageid=123456789, check=True)
    ...
    def test_constructor_no_title_or_pageid(self):
        with self.assertRaises(wikitools.exceptions.WikiError):
            page.Page(self.site, title="", pageid=0)
    ...
    def test_constructor_section_priority(self):
        p1 = page.Page(
            self.site, "Page#Section 2", sectionnumber=0, section="Section 1"
        )
    ...
    def test_protect(self):
        ...
        res = p1.protect(
            restrictions={"edit": "all", "move": "all"}, reason="test_protect"
        )
...
class TestPageList(unittest.TestCase):
    ...
    def test_listFromTextList_nocheck(self):
        ...
        api.logging = True
        pages = pagelist.listFromTextList(
            self.site, sequence=titles, datatype="titles", check=False
        )
        for item in pages:
            self.assertIsInstance(item, page.Page)
    ...
    def test_listFromTextList_pageids(self):
        ...
        api.logging = True
        pages = pagelist.listFromTextList(
            self.site, sequence=pageids, datatype="pageids", check=True
        )
        for item in pages:
            self.assertIsInstance(item, page.Page)
    ...
    def test_listFromTextList_titles(self):
        ...
        api.logging = True
        pages = pagelist.listFromTextList(
            self.site, sequence=titles, datatype="titles", check=True
        )
        for item in pages:
            self.assertIsInstance(item, page.Page)
            self.assertIsNot(item.exists, None)
    ...
    def test_listFromTextList_dbkeys(self):
        ...
        api.logging = True
        pages = pagelist.listFromTextList(
            self.site, sequence=dbkeys, datatype="dbkeys", check=True
        )
        for item in pages:
            self.assertIsInstance(item, page.Page)
            self.assertIsNot(item.exists, None)
