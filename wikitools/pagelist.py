...
def listFromTextList(site, sequence, datatype, check=True, followRedir=False):
    ...
    if datatype not in ["titles", "pageids", "dbkeys"]:
        raise wikitools.exceptions.WikiError(
            "listFromTextList datatype must be one of titles, pageids, dbkeys"
        )
    if datatype == "dbkeys":
        sequence = sequence.copy()
        sequence = [
            site.namespaces[int(i[0])]["*"] + ":" + i[1] if int(i[0]) else i[1]
            for i in sequence
        ]
        datatype = "titles"
    if not check:
        ...
        return [
            wikitools.page.Page(
                site, check=False, followRedir=followRedir, **{opt: item}
            )
...
def makePage(result, site, followRedir):
    ...
    if ns == site.NS_CATEGORY:
        item = wikitools.category.Category(
            site, title=title, check=False, followRedir=followRedir, pageid=pageid
        )
    elif ns == site.NS_FILE:
        item = wikitools.wikifile.File(
            site, title=title, check=False, followRedir=followRedir, pageid=pageid
        )
    else:
        item = wikitools.page.Page(
            site, title=title, check=False, followRedir=followRedir, pageid=pageid
        )
