...
class APIRequest:
    ...
    def __init__(self, site, data, write=False, multipart=False):
        ...
        if "maxlag" not in self.data and site.maxlag >= 0:
    ...
    def queryGen(self):
        ...
        while True:
            ...
            if "error" in data:
                if self.iswrite and data["error"]["code"] == "blocked":
                    raise exceptions.UserBlocked(data["error"]["info"])
                raise exceptions.APIQueryError(
    ...
    def __getRaw(self):
        ...
        while not data:
            try:
                ...
                data = self.response = self.site.session.post(
                    self.site.apibase,
                    data=self.data,
                ...
                warnstring = "%s: %s trying request again in %d seconds" % (
                    errname,
                    exc,
                    self.sleep,
                )
    ...
    def __parseJSON(self, data):
        try:
            ...
            if "error" in content:
                ...
                if error == "maxlag":
                    lagtime = int(
                        re.search("(\d+) seconds", content["error"]["info"]).group(1)
                    )
            ...
            if self.sleep >= self.site.maxwaittime or self.iswrite:
                raise exceptions.APIFailure(
                    "Invalid JSON received. API is broken, or this isn't a MediaWiki API"
                )
            warnstring = "Invalid JSON, trying request again in %d seconds" % (
                self.sleep
            )
            warnings.warn(warnstring, UserWarning)
            time.sleep(self.sleep + 0.5)
...
def resultCombine(type, old, new):
    ...
    """
    warnings.warn(
        "resultCombine is deprecated, will not be mantained, and may be removed in the future",
        DeprecationWarning,
    )
    ret = old
    if type in new["query"]:  # Basic list, easy
        ...
        for key in list(new["query"]["pages"].keys()):  # Go through each page
            if key not in old["query"]["pages"]:  # if it only exists in the new one
                ...
                if type not in new["query"]["pages"][key]:
                    continue
                elif (
                    type in new["query"]["pages"][key]
                    and type not in ret["query"]["pages"][key]
