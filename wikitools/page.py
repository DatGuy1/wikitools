    ...
    if len(bits) == 1 or bits[0] == "":
        ...
        for ns in site.namespaces:
            if nsprefix == site.namespaces[ns]["*"].lower():
                return int(ns)
            if (
                "canonical" in site.namespaces[ns]
                and nsprefix == site.namespaces[ns]["canonical"].lower()
            ):
                return int(ns)
        else:
            if site.NSaliases:
...
class Page(object):
    ...
    def __init__(
            ...
            if self.namespace is None and self.title:
                ...
                if self.namespace is not 0:
                    ...
                    if case == "first-letter":
                        self.unprefixedtitle = (
                            self.unprefixedtitle[0].upper() + self.unprefixedtitle[1:]
                        )
        ...
        if self.title:
            self.urltitle = (
                urllib.parse.quote(self.title).replace("%20", "_").replace("%2F", "/")
            )
        if self.pageid < 0:
            self.pageid = 0
    ...
    def isRedir(self):
        ...
        params = {"action": "query", "redirects": "", "pageids": self.pageid}
        req = api.APIRequest(self.site, params)
        res = req.query(False)
    ...
    def getHistory(self, direction="older", content=True, limit="all", user=None):
        ...
        if content:
            rvprop += "|content"
        return internalfunctions.getList(
            self,
            "prop",
            "revisions",
            "rv",
    ...
    def getHistoryGen(self, direction="older", content=True, limit="all", user=None):
        ...
        if content:
            rvprop += "|content"
        return internalfunctions.getListGen(
            self,
            "prop",
            "revisions",
            "rv",
    ...
    def edit(self, *args, **kwargs):
        ...
        if not self.title:
            self.setPageInfo()
        if "section" not in kwargs and self.section is not None:
    ...
    def move(
        ...
        if self.exists is False:
            raise exceptions.NoPage
        token = self.site.getToken("csrf")
        params = {"action": "move", "to": mvto, "token": token, "fromid": self.pageid}
        if reason:
    ...
    def purge(self, forcelinkupdate=False, forcerecursivelinkupdate=False):
        """
        Purge the page
        forcelinkupdate - Update the links tables and do other secondary data updates
        forcerecursivelinkupdate - Same as forcelinkupdate, and update the links tables for any page that uses this page as a template
        """
        if self.exists is None:
            self.setPageInfo()
        if self.exists is False:
    ...
    def delete(self, reason="", watch=False, unwatch=False, watchlist="preferences"):
        ...
        if self.exists is False:
            raise exceptions.NoPage
        token = self.site.getToken("csrf")
        params = {"action": "delete", "token": token, "pageid": self.pageid}
        if reason:
            params["reason"] = reason
        if watch:
            params["watch"] = "1"
