...
def getList(
    ...
    **kwargs
    ...
    qc = None
    while True:
        res, qc = __getListInternal(
            obj, querytype, listtype, prefix, direction, limit, qc, kwargs
        )
        entries = entries + res
...
def getListGen(
    obj, querytype, listtype, prefix, direction="older", limit="all", **kwargs
):
    if "continue" not in obj.site.features:
    ...
    while True:
        res, qc = __getListInternal(
            obj, querytype, listtype, prefix, direction, 1, qc, kwargs
        )
        yield res[0]
        count += 1
...
def __getListInternal(
    obj, querytype, listtype, prefix, direction, limit, querycontinue, kwargs
):
